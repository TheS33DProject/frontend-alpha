import { Currency, CurrencyAmount, Pair, Route, TokenAmount, Trade, Price, Percent } from '@pancakeswap/sdk'
...
export function useIdoState(): AppState['swap'] {
...
// from the current IDO inputs, compute swap details.
export function useIdoSwapInfo(
  independentField: Field,
  typedValue: string,
  inputCurrency: Currency | undefined,
  outputCurrency: Currency | undefined,
  s33drates: { of1s33d: number; of1usdt: number },
  recipient: string,
): {
  currencies: { [field in Field]?: Currency }
  currencyBalances: { [field in Field]?: CurrencyAmount }
  parsedAmounts: { INPUT: CurrencyAmount; OUTPUT: CurrencyAmount } | undefined
  idoTrade: Trade | undefined
  inputError?: string
} {
  const { account } = useWeb3React()
  const { t } = useTranslation()
  ...
  const isExactIn: boolean = independentField === Field.INPUT
  const calcTradeExactIn = (Number(Number(typedValue).toFixed(0)) * Number(s33drates.of1s33d)).toString()
  const calcTradeExactOut = (Number(typedValue) / Number(s33drates.of1s33d)).toString()

  // build Amounts for next step
  const parsedAmounts = {
    [Field.INPUT]: isExactIn
      ? tryParseAmount(typedValue, inputCurrency)
      : tryParseAmount(calcTradeExactIn, inputCurrency),
    [Field.OUTPUT]: isExactIn
      ? tryParseAmount(calcTradeExactOut, outputCurrency)
      : tryParseAmount(typedValue, outputCurrency),
  }
  ...
  // compare input balance to max input based on version
  const [balanceIn, amountIn] = [currencyBalances[Field.INPUT], parsedAmounts ? parsedAmounts.INPUT : null]
  ...
  const tokenIn = mainnetTokens[inputCurrency ? inputCurrency.symbol.toLowerCase() : 'usdt']
  const tokenOut = mainnetTokens[outputCurrency ? outputCurrency.symbol.toLowerCase() : 's33d']
  const inputInWei = typedValue
    ? isExactIn
      ? getDecimalAmount(new BigNumber(typedValue)).toString()
      : getDecimalAmount(new BigNumber(calcTradeExactIn)).toString()
    : '0'
  const outputInWei = typedValue
    ? isExactIn
      ? getDecimalAmount(new BigNumber(calcTradeExactOut)).toString()
      : getDecimalAmount(new BigNumber(typedValue)).toString()
    : '0'

  const tokenAmountIn = new TokenAmount(tokenIn, inputInWei)
  const tokenAmountOut = new TokenAmount(tokenOut, outputInWei)
  // console.log('decimalAmount:', getDecimalAmount(new BigNumber(typedValue)).toString())
  // console.log('tokenstuff:', tokenAmountIn.toExact(), tokenAmountOut.toExact())

  const pair = new Pair(tokenAmountIn, tokenAmountOut)
  const route = inputCurrency && outputCurrency ? new Route([pair], inputCurrency, outputCurrency) : undefined
  const idoTrade = {
    executionPrice:
      inputCurrency && outputCurrency ? new Price(inputCurrency, outputCurrency, inputInWei, outputInWei) : undefined,
    inputAmount: tokenAmountIn,
    outputAmount: tokenAmountOut,
    route: route ?? undefined,
    tradeType: isExactIn ? 0 : 1,
    nextMidPrice:
      inputCurrency && outputCurrency ? new Price(inputCurrency, outputCurrency, inputInWei, outputInWei) : undefined,
    priceImpact: new Percent(1, 1),
    maximumAmountIn: () => parsedAmounts[Field.INPUT] ?? tryParseAmount('0', inputCurrency),
    minimumAmountOut: () => parsedAmounts[Field.INPUT] ?? tryParseAmount('0', inputCurrency),
  }
